...
当前正式发布与安全修复范围覆盖 **Windows 11 桌面端** 和 **Web Preview**。仓库中的 Tauri 依赖图可能在 `Cargo.lock` 中包含 macOS/Linux 等上游跨平台依赖，但除非发布说明中明确提供对应平台构建，这些平台不属于当前正式支持或发布范围。

对于仅存在于未发布平台代码路径中的第三方依赖告警，维护者会记录评估依据，并持续跟踪上游 Tauri/Wry/GTK 依赖更新；一旦项目开始发布对应平台版本，该类依赖将重新进入正式修复范围。
