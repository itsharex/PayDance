# 安全策略

> [English version →](../docs/SECURITY_EN.md)

## 受支持的版本

仅最新正式版接受安全修复。旧版本不受支持。

| 版本     | 支持状态               |
| -------- | ---------------------- |
| 最新版   | :white_check_mark: |
| 旧版本   | :x:                |

## 受支持的平台

当前正式发布与安全修复范围覆盖 **Windows 11 桌面端** 和 **Web Preview**。仓库中的 Tauri 依赖图可能在 `Cargo.lock` 中包含 macOS/Linux 等上游跨平台依赖；这些依赖不会被简单忽略，但在对应平台尚未正式发布前，会按“未发布平台路径”评估风险、记录依据并跟踪上游。

## 报告漏洞

**请勿通过公开 GitHub Issue 报告安全漏洞。**

请通过以下任一方式私下报告：

- **GitHub 安全通告**（推荐）：访问 [Security Advisories](https://github.com/MasterBao66/PayDance/security/advisories/new) 提交
- **电子邮件**：查看 [作者 GitHub 首页](https://github.com/MasterBao66) 上公示的邮箱（请在主题中注明 `[SECURITY]`）

### 报告中应包含

- 漏洞描述
- 复现步骤
- 受影响版本
- 你已发现的任何缓解措施

### 预期流程

- **确认**：72 小时内
- **评估**：5 个工作日内
- **修复时间**：取决于严重程度；高危漏洞将尽快修复
- **披露**：修复版本发布并给用户留出合理更新时间后

## 敏感信息

- **切勿**在公开 Issue 中粘贴薪资数据、配置文件或日志
- **切勿**在任何公开渠道中泄露私钥、签名密钥或其它凭据
- 薪跳 PayDance 将薪资配置保存在本机应用数据目录下的 `salary-settings.json`，该文件含有个人信息，不应分享

## 签名密钥泄露

若 Tauri 更新签名密钥发生泄露：

1. 立即撤销已泄露的密钥
2. 生成新的密钥对
3. 用新公钥更新 `tauri.conf.json`
4. 发布用新密钥签名的版本
5. 旧版本将无法再通过自动更新升级——用户需手动下载

## 本地数据

薪跳 PayDance 是一款本地优先应用。薪资数据、工作时间和偏好设置仅保存在你的设备上，不会发送到任何远程服务器。如果你发现可能导致本地配置遭到未授权访问的漏洞，请立即报告。
