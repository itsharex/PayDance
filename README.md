...
薪跳 PayDance 是一款面向 Windows 11 的桌面实时薪资仪表盘。设置薪资、工作日、上下班时间和午休规则后，它会在桌面上实时显示今天已经挣到的钱，让每一秒的收入跳动都看得见。

它不是一次性算完即关的工具，而是一个安静常驻的工资看板：主窗口用于查看今日入账、工作进度和预计收入，迷你悬浮窗口用于把核心金额放在屏幕角落，随时瞥一眼就知道今天已经入账多少。
...
- 打开主窗口：查看今日入账、已工作、距离下班、今日预计和今日进度。
- 进入迷你悬浮：只保留 `¥123.45`，默认置顶，适合放在屏幕角落。
- 右键迷你窗口：调节透明度，支持 `10% - 100%`，金额本身始终保持清晰。
- 关闭主窗口：隐藏到系统托盘，左键托盘图标可唤起主界面，右键打开菜单。
...
| 模块 | 能力 |
| --- | --- |
| 首次启动向导 | 分步配置薪资模式、工作时间和使用偏好，可选择主题、置顶、迷你悬浮和开机自启动 |
| 实时薪资计算 | 根据当前时间计算今日已挣金额、今日预计收入、工作进度、已工作时长和剩余时间 |
| 薪资说明 | 展示日薪、时薪、分薪和秒薪换算结果，避免主界面承载过多次级信息 |
| 工作日识别 | 非工作日显示今日休息，避免周末继续累计 |
| 午休剔除 | 可选择剔除午休时间，让收入累计更贴近真实工时 |
| 夜班支持 | 支持跨零点上下班时间，并按实际工作段判断“正在上班 / 正在夜班” |
| 设置中心 | 保持独立小卡片布局，支持修改薪资模式、薪资、每周工作日、工作时间、午休和金额变换 |
| 迷你悬浮 | 支持拖拽移动、双击恢复主窗口、右键调节透明度、记忆迷你窗口尺寸 |
| 桌面体验 | 支持窗口置顶、最小化、关闭到托盘、无边框拖拽和开机自启动 |
| 作者入口 | 设置中心底部展示作者署名、版本号、GitHub 仓库入口和版权信息 |
...
| 数据 | 用途 |
| --- | --- |
| 薪资与薪资模式 | 计算日薪、时薪、分薪、秒薪和今日入账 |
| 工作日与上下班时间 | 判断当前是否上班、休息、午休或夜班 |
| 午休时间 | 从工作时长中剔除休息段 |
| 主题、置顶、窗口尺寸 | 恢复上次使用的桌面状态 |
| 迷你悬浮透明度 | 恢复迷你窗口的视觉透明度 |
...
- 版本一致性：`package.json`、`package-lock.json`、`tauri.conf.json`、`Cargo.toml`、`Cargo.lock` 保持同步。
- 仓库卫生：CI 会扫描旧品牌文案、敏感信息和不应提交的研发痕迹。
...
- 透明度面板右侧百分比改为与“透明度”标题同色、同字重，避免 `100%` 在浅色和深色模式下显得像次要说明。
- 补充透明度面板文字层级测试，防止百分比再次被改成弱化灰色。
- 补充高 DPI 与非零屏幕工作区下的迷你透明度面板定位回归测试，明确允许奇数物理像素宽度带来的半像素中心误差。
- README 重整为更清晰的产品页：强化核心体验、功能矩阵、隐私数据边界、工程质量和发布流程说明。

### v0.7.7

- 修正 v0.7.6 透明度面板仍可能没有与迷你悬浮窗口居中对齐的问题：主定位路径改为读取主窗口内容区 `innerPosition` / `innerSize`，以用户实际看到的迷你窗口为锚点。
- 透明度面板定位不再假设自身尺寸固定等于配置值，打开前会重置为 `108 × 52`，再读取面板内容区尺寸与内外窗口偏移，最后反推出正确的外层窗口坐标。
- 补充内容区对齐回归测试，覆盖 Windows 无边框透明窗口可能存在不可见外框或内容区偏移的场景。
...
- 透明度面板打开逻辑增加兜底路径：若窗口几何信息读取失败，会立即退回到鼠标位置附近显示，避免右键无反应。
- 透明度面板继续压缩到 `108 × 52`，减少浮层存在感，同时保留细滑轨和小圆点的可操作性。
- 为窗口权限、右键兜底定位和透明度面板尺寸补充回归测试，防止后续桌面 API 调整再次漏配权限。

### v0.7.3

- 迷你悬浮透明度面板重做为 `116 × 60` 的小型浮层，滑轨和圆点明显收窄，避免上一版横向大胶囊带来的生硬感。
- 透明度面板改为跟随迷你悬浮窗口居中显示，空间不足时自动显示到上方，并限制在当前屏幕工作区内。
- 今日入账金额脉冲光改为单层 `14px × 30px` 光晕，范围比 v0.7.1 更容易感知，持续时间保持 `220ms`，不再叠加第二层环境光。
- 为透明度面板尺寸、位置计算和金额脉冲补充回归测试，防止后续迭代重新滑回大面板、双层发光或鼠标坐标漂移。
