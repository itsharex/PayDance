...
- 仪表盘 `h` / `m` / `¥` 的字号与间距继续优化，今日预计的 `¥` 与金额数字等高，同时保留原有进度条圆点光感。
- 薪资说明弹窗和设置中心中的数字、英文与符号统一使用仪表盘字体，减少不同区域之间的视觉割裂。
- 保持顶部按钮激活态重量不变，仅修正当前版本中会影响观感一致性的边缘与字体细节。

### v0.6.8

- 主窗口移除外层浅灰边距后，标题栏状态区与窗口操作区整体向内、向下调整，避免贴近圆角边缘。
- 深色模式仪表盘去除顶部泛白径向高光，改为更干净的深色层次、细边框和柔和阴影。
- 仪表盘继续保持“两层”结构：金额层 + 单个仪表盘层，三项指标内部不再恢复独立小卡片。
- 今日进度条保留平面轨道，同时恢复 0.6.6 深色模式下的三段金色填充质感。
- “薪资说明”入口优化为低存在感胶囊按钮，Hover 只给轻微暖色反馈，不增加新的厚重层级。
- 仪表盘中的 `h / m / ¥` 与数字拉开间距，提升小尺寸下的可读性和精致感。
- 主题切换时先同步 Web 主题壳层，再同步原生窗口主题，并加入短暂表面遮罩，减少四角旧主题残影。

### v0.6.7

- 新增桌面端单实例保护，重复启动时聚焦已有主窗口，避免双开后两个进程主题状态不同步。
- 主题切换保留统一入口、点击锁和乱序保护，但移除全局视觉 transition 冻结，减少不必要的副作用。
- 主界面仪表盘回到全平面设计，取消深色玻璃面、径向高光和强阴影。
- “薪资说明”Hover 改为全平面反馈，深色模式使用略亮深灰底和细橙边框，不再使用玻璃光晕。
- 今日进度条取消深色凹槽轨道，保留 0.6.6 中更清晰的深色圆点质感。

### v0.6.6

- 修复浅色 / 深色模式快速切换时前端主题与原生窗口主题偶发不同步的问题，主题应用改为统一入口并加入切换锁与乱序保护。
- 主题切换期间短暂冻结视觉 transition，减少透明窗口与毛玻璃背景叠色造成的灰蒙中间态。
- 深色模式“薪资说明”入口 Hover 同步升级为深色玻璃底、低透明橙色描边和柔和光晕。
- 深色主界面仪表盘改用更深的墨色玻璃面、细高光边和更克制的阴影，降低灰盒子廉价感。
- 今日进度条改为主题专属轨道、填充和圆点变量，深色模式下轨道更像凹槽，暖金填充更收敛。
